...
│   │   ├── multi_horizon_correlation.py     探索性：net_sentiment vs. 多種天數的累積報酬率（不 append 進正式歷史記錄）
│   │   ├── aggregation_method_comparison.py 探索性：比較不同的每日新聞情緒聚合方式（同上，不進正式歷史記錄）
...
1. **樣本量已經跨過門檻，但看不出統計上顯著的關聯**：加入 AMD/TSM/QCOM 之後，4 支台股的新聞
   ...
   1-2 次 buy。**這不再是「樣本不夠所以看不到」的問題**——資料變多之後，關聯反而更不顯著。

   進一步測試了兩個最可能的「方法論本身有問題」假說，**都被推翻了**：
   - `multi_horizon_correlation.py`：會不會效果要累積幾天才顯現，只看隔天看不到？測了
     1/3/5/10/20 天共 20 個組合，**0 個達到 p<0.05**，相關係數不隨時間窗拉長而變強。
   - `aggregation_method_comparison.py`：會不會是「當天所有新聞情緒簡單平均」把訊號稀釋掉了
     （66% 新聞被判成 neutral，見下一項）？測了排除 neutral、信心度加權、用則數比例算三種
     替代聚合方式，共 80 個組合，**同樣 0 個達到 p<0.05**，|r| 都維持在 0.16 以內。

   兩輪加起來 100 個組合裡沒有任何一個顯著，已經不太像是運氣或方法論細節的問題。比較誠實的
   結論：**目前這種「美股半導體供應鏈新聞情緒 → 台股隔天/未來報酬率」的訊號設計，看不出可偵測的
   線性關聯**。值得一提的但書：新聞提到的公司常常不是股價本身（跨市場對齊的固有限制，見
   「內部人賣股新聞觀察分析」一節也提過同樣問題），如果要再往下查，下一步應該換一個更根本的
   假設去測，而不是繼續在同一組訊號上換聚合方式——例如改用台股自己語言/市場的新聞，而不是
   美股供應鏈新聞當代理指標。
...
2. **情緒分類模型**：`train.py` 目前是正式訓練版本，用完整的
   ...
   的限制：
   - 訓練資料是「推文」語氣，面對「正式新聞標題」（陳述事實、較少表態）時，模型會把約 2/3
     判成 neutral。實測檢查過 18,615 則正式新聞的分類結果：**不是鑑別度低/模稜兩可**——
     `sentiment_neutral` 機率幾乎都落在接近 0 或接近 1（很少卡在中間），模型對每則新聞都給出
     很篤定的答案，抽樣檢查判斷也大致合理（"Stock Opened Up by 8.47%" → bullish、純陳述句
     → neutral）。真正的限制是：正式財經新聞標題本來就有不少是中性的事實陳述，不是每則都在
     講漲跌，這是新聞語料本身的組成，不是模型判斷失準。這也代表加大樣本量/epoch 或換訓練資料集
     不會改變太多——`net_sentiment` 測不出跟報酬率的關聯（見上一項），問題比較可能出在「把
     一天所有新聞情緒平均成一個數字」這個聚合方式本身，而不是情緒分類器本身不準。
   - 沒有針對特定產業/股票做領域微調。
